...
class TransUNetSE3D(nn.Module):
    def __init__(
        ...
        super(TransUNetSE3D,self).__init__()

        self.Maxpool = nn.MaxPool3d(kernel_size=2,stride=2)

        self.Conv1 = conv_block_3D(ch_in=img_ch,ch_out=8)
        self.SE1 = ChannelSELayer3D(num_channels=8)

        self.Conv2 = conv_block_3D(ch_in=8,ch_out=16)
        self.SE2 = ChannelSELayer3D(num_channels=16)

        self.Conv3 = conv_block_3D(ch_in=16,ch_out=32)
        self.SE3 = ChannelSELayer3D(num_channels=32)

        self.Conv4 = conv_block_3D(ch_in=32,ch_out=64)
        self.SE4 = ChannelSELayer3D(num_channels=64)

        self.Conv5 = conv_block_3D(ch_in=64,ch_out=128)

        self.Up5 = up_conv_3D(ch_in=128,ch_out=64)
        self.Up_conv5 = conv_block_3D(ch_in=128, ch_out=64)

        self.Up4 = up_conv_3D(ch_in=64,ch_out=32)
        self.Up_conv4 = conv_block_3D(ch_in=64, ch_out=32)

        self.Up3 = up_conv_3D(ch_in=32,ch_out=16)
        self.Up_conv3 = conv_block_3D(ch_in=32, ch_out=16)

        self.Up2 = up_conv_3D(ch_in=16,ch_out=8)
        self.Up_conv2 = conv_block_3D(ch_in=16, ch_out=8)
...
class ResTransUNet3D(nn.Module):
    def __init__(
        self,
        img_dim = 192,
        patch_dim = 8,
        ...
        super(ResTransUNet3D,self).__init__()

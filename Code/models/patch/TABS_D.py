...
from models.patch.Transformer import TransformerModel
from models.patch.PositionalEncoding import LearnedPositionalEncoding
...
class TABS_D(nn.Module):
    def __init__(
        ...
        self.mlp = Mlp(in_features=128*6*6*6, hidden_features=2048, out_features=2)
    def forward(self,x):
        # y is gt
        # rand_tensor = torch.rand_like(y) * 0.2 + 0.9
        # y = y * rand_tensor
        # encoding path
        x1 = self.Conv1(x)
        x2 = self.Maxpool(x1)

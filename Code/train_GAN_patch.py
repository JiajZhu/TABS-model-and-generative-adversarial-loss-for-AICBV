...
from models.patch.TABS_D import TABS_D
from models.patch.Unet_D import Unet_D
...
def main(config):
    ...
    if cont_training:
        ...
        # netD = Unet_D(UnetLayer = config.UnetLayer, img_ch = config.img_ch, output_ch = 1, first_layer_numKernel = config.first_layer_numKernel).to(config.device)
        netD=TABS_D(img_dim=96).to(config.device)
    else:
        netG = TABS(img_dim=96).to(config.device)
        # netG = ResAttU_Net3D(UnetLayer = config.UnetLayer, img_ch = config.img_ch, output_ch = 1, first_layer_numKernel = config.first_layer_numKernel).to(config.device)
        # netG = TransG(config).to(config.device)
    ...
    optimizerG = optim.SGD(netG.parameters(), lr=config.TABS_lr)
    optimizerD = optim.SGD(netD.parameters(), lr=config.Discriminator_lr)
    DPATH="/content/drive/MyDrive/SwinGan/experiment/GAN_TABS_patch/CheckPoint_3_model_D.pt"
    ...
    # GPATH="/content/drive/MyDrive/SwinGan/experiment/GAN_TABS_patch/CheckPoint_3_model_G.pt"
    ...
    with open(os.path.join(LossPath + '/learning_curve.csv'), 'a', encoding='utf-8', newline='') as f:
        wr = csv.writer(f)
        wr.writerow(['Current Epoch','Train errD','Train errG','Train img loss','Train PCC','Train SCC','Train ACC',\
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        # for step in tqdm(range(config.train_iteration)):#config.train_iteration
            ...
            real = (post_img-pre_img).to(config.device)
            # real[mask!=1]=0
            real = torch.mul(real.type(torch.cuda.FloatTensor), mask.type(torch.cuda.FloatTensor)).to(config.device)
            # print("real.shape",real.shape)
            b_size  = real.size(0)
            label = torch.full((b_size,), real_label,  device=config.device)
            # print("label",label)
            output = netD(real)
            acc_real = (output.argmax(dim=-1) == label.to(config.device)).float().mean()
            ...
            prediction = netG(pre_img.type(torch.cuda.FloatTensor).to(config.device))
            # prediction[mask!=1] = 0
            fake = prediction.clone()
            # fake[mask!=1]=0
            fake = torch.mul(fake.type(torch.cuda.FloatTensor), mask.type(torch.cuda.FloatTensor)).to(config.device)
            label.fill_(fake_label)
            output = netD(fake.detach())
            acc_fake = (output.argmax(dim=-1) == label.to(config.device)).float().mean()
            ...
            img_loss = torch.mean(criterion_img.lossfun((prediction_vector[mask_vector == 1].type(torch.cuda.FloatTensor) - target_vector[mask_vector == 1].type(torch.cuda.FloatTensor))[:,None]))
            # img_loss = criterion_img(prediction_vector[mask_vector == 1],target_vector[mask_vector == 1])
            # img_loss = criterion_img(prediction,target)
            errG = discriminator_loss_weight*criterionD(outputG, label) + img_loss_weight*img_loss
            ...
            if step%1 == 0:
                ...
                print(f"Gradient norms for D at iteration {step}: {grad_norms}")
                # print(f"Gradient norms for G at iteration {step}: {grad_norms_G}")
                print('steps {0:d} - errD {1:.4f} - errG {2:.4f} -img loss {3:.4f} -acc {4:.4f}'.format(step,errD.item(),errG.item(),img_loss,acc))
        ...
        print(f"[ Train | {epoch + 1:03d}/{max_epoch:03d} ] errD = {train_errD:.5f},errG = {train_errG:.5f},img loss = {train_img_loss:.5f}, train_acc ={train_acc:.5f}")
...
##################################################################################################################################
        ...
        with open(os.path.join(LossPath + '/learning_curve.csv'), 'a', encoding='utf-8', newline='') as f:
            wr = csv.writer(f)
            wr.writerow(['%d' %(epoch + 1), '%.15f' % train_errD, '%.15f' % train_errG, '%.15f' % train_img_loss,'%.15f' % train_pcc,'%.15f' % train_scc,'%.15f' % train_acc,\
